...
contract Hourglass {
    ...
    // no one can sell or transfer token until we've passed the hatch amount
    modifier hasHatched(uint256 _amountOfEthereum) {
        if (hatchOpen) {
            require(
                (totalEthereumBalance() - _amountOfEthereum) >= hatchQuota_,
                "Ethereum collateral supply has not reached hatch quota"
            );
            _;
        } else {
            // in case the ether count drops low, the hatch phase won't reinitiate
            hatchOpen = false;
            _;
        }
    }
    ...
    */
    ...
    function reinvest() public onlyStronghands() {
        ...
        // retrieve operational bonus
        _dividends += operationalBalance_[_customerAddress];
     ...
     */
    function exit() public hasHatched(0) {
        ...
        onlyBagholders()
    {
        ...
        uint256 _operationalCost = SafeMath.div(
            _undividedDividends,
            operationalFee_
        );
        ...
        // DAO takes a cut for operation fees
        operationalBalance_[DAOaddress] = SafeMath.add(
